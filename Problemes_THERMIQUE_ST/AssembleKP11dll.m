...
% AssembleK_P1_1dll.m                                
%
%construction de la matrice globale de conductivité
%**************************************************************

function [Kglob]=AssembleKP11dll(Connect,Nx,Ny,H,NbPtInt);

%1) Initialisation de la matrice de rigidite
%   globale

Kglob=sparse(zeros(  length(Nx) ));
...
for i=1:size(Connect,1)
    ...
    for l=1:NbPtInt
        ...
        [Be]=ConstruitBe1ddl(deriveesX,deriveesY,NoeudsDuTri);
        
        %2-4-3) Calcul des matrices élémentaire
        
        Ke=Be'*Be*poids*aireTrI;
        
        %2-4-4) Assemblage dans la matrice globale
        
        [Kglob]=AssembleKeiECOMixteEF_1dll(Ke,Kglob,...
            NoeudsDuTri);       
